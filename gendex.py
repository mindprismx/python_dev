...
<head>
    ...
    <style>
        ...
        pre {{
            ...
            margin: 0 !important;
            padding: 0 !important;
            /* ensure your text color still shows */
...
</head><body onload="document.getElementById('theDate').textContent = formatLinuxDate()">
...
</script><span class="prompt">𝜟·</span> ls -lhFgo /{path}
total {total_size}
{entries}
...
def get_mode_str(mode):
    """Use Python's stat.filemode for accurate Unix perms."""
...
def format_entry(mode, nlink, size, mtime, name, href, is_dir, full_path):
    ...
    # LS: if older than 6 months or more than 6 months in the future, show year
    now = datetime.datetime.now()
    six_months = datetime.timedelta(days=182)
    if mtime < now - six_months or mtime > now + six_months:
        # month, day, two spaces, year (→ total width 12)
        date_str = mtime.strftime("%b %e  %Y")
    else:
        # month, day, space, HH:MM (→ total width 12)
        date_str = mtime.strftime("%b %e %H:%M")
...
def build_index(dirpath, rel_dir, dirnames, filenames, include_dotfiles):
    ...
    # sort and filter out index.html
...
def process_dir(root, include_dotfiles=False, verbose=False):
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        if index_exists:
            ...
            # If this wasn’t one of our auto‐generated files, skip
            if TOKEN not in content:
                if verbose:
                    print(f"Skipping existing index (no token): {index_path}")
                continue

            # If the content hasn’t changed, skip
        ...
        # Otherwise we fall through and write out html
